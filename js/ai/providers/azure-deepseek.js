// Azure DeepSeek LLM Provider (OpenAI-compatible API)

import { LLMProvider } from './base.js';

export class AzureDeepSeekProvider extends LLMProvider {
  constructor(apiKey, resource = 'hellobusiness999-4411-resource', model = 'DeepSeek-V3.2') {
    super(apiKey);
    this.model = model;
    this.resource = resource;
    this.baseUrl = `https://${resource}.openai.azure.com/openai/v1/chat/completions`;
  }
  ...
  async call(systemPrompt, userPrompt, tools) {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Azure DeepSeek API error: ${error.error?.message || response.statusText}`);
    }
  ...
  async test() {
    ...
    const data = await response.json();
    console.log('Azure DeepSeek test successful:', data.choices[0].message.content);
    return true;
  }
}
